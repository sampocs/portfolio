import { Asset, PerformanceData, PortfolioSummary, CategoryAllocation, AllocationDelta } from './types';

export const calculatePortfolioSummary = (positions: Asset[]): PortfolioSummary => {
  const totalValue = positions.reduce((sum, asset) => sum + parseFloat(asset.value), 0);
  const totalCost = positions.reduce((sum, asset) => sum + parseFloat(asset.cost), 0);
...
export const filterAssetsByCategory = (assets: Asset[], category: 'Crypto' | 'Stocks'): Asset[] => {
  if (category === 'Crypto') {
...
export const getLatestPerformanceData = (performanceData: PerformanceData[]): PerformanceData | null => {
  if (performanceData.length === 0) return null;
  return performanceData[performanceData.length - 1];
};

export const getPerformanceRange = (performanceData: PerformanceData[]): { min: number; max: number } => {
...
export const aggregateAssetsByCategory = (assets: Asset[]): CategoryAllocation[] => {
  const categoryMap = new Map<string, CategoryAllocation>();
  const totalPortfolioValue = assets.reduce((sum, asset) => sum + parseFloat(asset.value), 0);
  ...
  const categories = Array.from(categoryMap.values()).map(category => {
    ...
    const percentageDelta = category.currentAllocation - category.targetAllocation;
...
export const calculateAllocationDelta = (current: number, target: number, totalValue: number): AllocationDelta => {
...
export const getCategoryColor = (category: string): string => {
  const categoryColors: { [key: string]: string } = {
    ...
    'Crypto Tokens': '#93C5FD', // Very light blue
    'Gold': '#EF4444', // Red
